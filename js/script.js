...
class Vec3 {
	...
	distTo(p) {
		return Math.hypot(this.x - p.x, this.y - p.y, this.x - p.z);
	}
	clone() {
		return new Vec3(this.x, this.y, this.z, this.r);
	}
	updateRotation() {
		let { x, y, z } = this.clone().minus(origin);
		this.screenCoord = [x * cx - y * sx, (y * cx + x * sx) * cy + sy * z];
...
let nextPoint = () => {
	...
	d = lerp(d, 150, 0.001);
	d += (noise(step / 100, 1e6) * 2 - 1) * 0.1;
	d = constrain(d, 100, 200);
	t += (noise(step / 200, 1e7) * 2 - 1) * 0.02;
	p += (noise(step / 200, 1e8) * 2 - 1) * 0.02;
